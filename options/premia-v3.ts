...
const fetch = async (options: FetchOptions) => {
  const todayBlock = (await getBlock(options.startOfDay, options.chain, {}))
  const yesterdayBlock = (await getBlock(options.startOfDay - 60 * 60 * 24, options.chain, {}))
  const query = gql`
  {
  ...
  `
  const response: any = await request(v3Endpoints[options.chain], query);
...
const adapter: SimpleAdapter = {
  ...
  methodology: {
    ...
    SupplySideRevenue: "Liquidity providers collect 50% of the taker fees and earn revenue from market-making options.",
