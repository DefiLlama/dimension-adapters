...
const getAdenDailyFeesFromAster = async () => {
  const response = (await httpGet(
    "https://www.asterdex.com/bapi/futures/v1/public/future/aden/fee/all"
  )) as {
    data: {
      adenFeeVOFor24Hour: {
        fee: number;
  ...
  return {
    dailyFees: response.data.adenFeeVOFor24Hour.fee,
    dailyRevenue: response.data.adenFeeVOFor24Hour.revenue,
    dailyProtocolRevenue: response.data.adenFeeVOFor24Hour.revenue,
  };
};
