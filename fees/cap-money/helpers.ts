...
export const isKnownVault = (options: FetchOptions, vault: string) => {
    const vaults = vaultsSymbols.map(symbol => capConfig[options.chain].tokens[symbol]);
    return vaults.map(vault => vault.address.toLowerCase()).includes(vault);
}

export const fetchAssetAddresses = async (options: FetchOptions, chain: string) => {
...
export const fetchVaultConfigs = async (options: FetchOptions) => {
    ...
    const vaultConfigsByAsset: Record<string, { asset: string, vault: string, interestReceivers: string[] }> = {};
    for (const event of assetAddedEvents) {
        ...
        if (!vaultConfigsByAsset[asset]) {
            ...
            throw new Error(`Vault mismatch for asset ${asset}: ${vaultConfigsByAsset[asset].vault} !== ${vault}`);
        }
    ...
    const insuranceFunds: string[] = (await options.api.batchCall(
    ...
    const result = arrayZip(vaultConfigs, insuranceFunds)
        .map(([vaultConfig, insuranceFund]) => ({
            ...vaultConfig,
            insuranceFund: devAddresses.includes(insuranceFund) ? null : insuranceFund,
        }))
    return result;
}