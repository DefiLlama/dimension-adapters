...
const contracts: Record<string, string[]> = {
    ...
    [CHAIN.ZKSYNC]: ["0x8A1142620CbdE2f2d63E88F35D0D76eAAce0AC9e"],
};
...
const fetch = async (options: FetchOptions): Promise<FetchResultV2> => {
  const dailyFees = options.createBalances();
    ...
    const results = await queryDuneSql(options, `
        ...
        WHERE "to" IN (${contracts[options.chain].join(",")})
          AND TIME_RANGE
      `);

    if (results && results.length > 0) {
        results.forEach(row => {
            dailyFees.addGasToken(row.fees || 0);
        });
    }
...
const adapter: Adapter = {
  ...
  adapter: {
    ...
    [CHAIN.ZKSYNC]: { fetch, start: '2023-11-20', },
  },
};
