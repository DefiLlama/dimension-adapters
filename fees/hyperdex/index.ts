...
//
// LI.FI's router pays the fee out in the same transaction and emits FeesForwarded with the full
// recipient split (see fees/lifi): one leg to LI.FI's own address, one leg to the integrator's
// registered payout address. HyperDex's fee is the leg paid to HYPERDEX_FEE_RECIPIENT; LI.FI's leg
// is LI.FI's revenue and is counted by the lifi adapter, not here.
//
...
// recipient on every chain, at 50/50 with LI.FI's leg.
//
// Forwarder addresses: https://github.com/lifinance/contracts/tree/main/deployments (FeeForwarder),
...
const chainConfig: Record<string, { forwarders: string[]; start: string }> = {
  [CHAIN.ROBINHOOD]: {
...
const fetch = async (options: FetchOptions) => {
  const { forwarders } = chainConfig[options.chain]
  ...
  logs.forEach((log: any) => {
    if (blacklistForChain.has(String(log.token).toLowerCase())) return
    log.fees.forEach((fee: any) => {
      if (HYPERDEX_FEE_RECIPIENTS.includes(String(fee.recipient).toLowerCase())) dailyFees.add(log.token, fee.amount)
    })
  })
...
const adapter: SimpleAdapter = {
  ...
  adapter: chainConfig,
  methodology: {
    Fees: 'The 0.25% integrator fee HyperDex charges on every swap, bridge and basket order routed through LI.FI, read from the FeesForwarded leg paid to HyperDex.',
