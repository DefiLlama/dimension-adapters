...
export async function getPoolFees(pool: AaveLendingPoolConfig, options: FetchOptions, balances: {
  ...
  for (let reserveIndex = 0; reserveIndex < reservesList.length; reserveIndex++) {
    ...
    const reserveAddress = normalizeAddress(reservesList[reserveIndex])
    const isUSDXL = reserveAddress.toLowerCase() === USDXL_TOKEN.toLowerCase()
    if (isUSDXL) {
      totalLiquidity = BigInt(reserveDataBefore[reserveIndex].totalVariableDebt)
    } else {
