...
function getFeeRecipient(timestamp: number) {
  ...
  if (timestamp <= 1743465600) {
    return '0x63695Eee2c3141BDE314C5a6f89B98E62808d716'
  } else {
    return '0xE344241493D573428076c022835856a221dB3E26'
  }
}
...
const fetch: any = async (options: FetchOptions) => {
  const dailyFees = await getETHReceived({ options, target: getFeeRecipient(options.fromTimestamp) })
