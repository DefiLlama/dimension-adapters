...
type Cursor = { data: Buffer; bits: number; pos: number }

const left = (c: Cursor) => c.bits - c.pos

const uint = (c: Cursor, n: number): number => {
  if (c.pos + n > c.bits) throw new Error('short read')
  let v = 0
  for (let i = 0; i < n; i++) {
    const bit = c.pos + i
    v = v * 2 + ((c.data[bit >> 3] >> (7 - (bit & 7))) & 1)
  }
  c.pos += n
  return v
}

const big = (c: Cursor, n: number): bigint => {
  if (c.pos + n > c.bits) throw new Error('short read')
  let v = 0n
  for (let i = 0; i < n; i++) {
    const bit = c.pos + i
    v = (v << 1n) | BigInt((c.data[bit >> 3] >> (7 - (bit & 7))) & 1)
  }
  c.pos += n
  return v
}

const coins = (c: Cursor): bigint => {
  const len = uint(c, 4)
  return len === 0 ? 0n : big(c, len * 8)
}

const addrStd = (c: Cursor): boolean => {
  if (left(c) !== 267) return false
  if (uint(c, 2) !== 2) return false // addr_std$10
  if (uint(c, 1) !== 0) return false // no anycast
  uint(c, 8)
  big(c, 256)
  return left(c) === 0
}

// Same check, keeping the address. Burner logs carry the payer, and which payer it was is the
// difference between a fee a validator was charged and the trading bot's own profit.
const addrStdRaw = (c: Cursor): string | null => {
  if (left(c) !== 267) return null
  if (uint(c, 2) !== 2) return null // addr_std$10
  if (uint(c, 1) !== 0) return null // no anycast
  const wc = uint(c, 8)
  const hash = big(c, 256)
...
const readCoinsThenAddress = (b64: string, n: number): bigint[] | null => {
  ...
  if (!cell) return null
  try {
    const c: Cursor = { data: cell.data, bits: cell.bits, pos: 0 }
    uint(c, 32) // round_since
    const v: bigint[] = []
    for (let i = 0; i < n; i++) v.push(coins(c))
    return addrStd(c) && sharesFitRepayment(v) ? v : null
  } catch { return null }
}
...
const readAddressThenCoins = (b64: string, n: number): bigint[] | null => {
  ...
  try {
    const c: Cursor = { data: cell.data, bits: cell.bits, pos: 0 }
    uint(c, 32) // round_since
    if (uint(c, 2) !== 2 || uint(c, 1) !== 0) return null
    uint(c, 8)
    big(c, 256)
    const v: bigint[] = []
    for (let i = 0; i < n; i++) v.push(coins(c))
    return left(c) === 0 && sharesFitRepayment(v) ? v : null
  } catch { return null }
}
...
const readReceived = (b64: string): Receipt | null => {
  ...
  try {
    const c: Cursor = { data: cell.data, bits: cell.bits, pos: 0 }
    uint(c, 32) // op, whatever the payer happened to send
    const value = coins(c)
    const total = coins(c)
    const src = addrStdRaw(c)
