...
const fetch = async (options: FetchOptions) => {
  ...
  const dailySupplySideRevenue = totalYield.clone(1 - PERFORMANCE_FEE_RATE);
  const dailyRevenue = options.createBalances();
  dailyRevenue.addBalances(totalYield.clone(PERFORMANCE_FEE_RATE));
  ...
  vaultLogs.forEach((log) => {
    if (log.feeAmount > 0n) {
      dailyRevenue.add(asset, log.feeAmount);
    }
  });
