...
import { base58Encode, getProgramAccounts, readI80F48 } from "../../helpers/solana";
...
// Public Solana endpoints rate limit getProgramAccounts; the sdk backs off and retries
// (5 attempts) rather than reporting a short day.
async function getBanks(): Promise<Buffer[]> {
  ...
  return accounts.map((account) => Buffer.from(account.account.data[0], "base64"));
}
...
const fetch = async (options: FetchOptions) => {
  ...
  for (const bank of banks) {
    const mint = base58Encode(bank.subarray(OFF.mint, OFF.mint + 32));
