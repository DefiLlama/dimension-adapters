...
const PERFORMANCE_AND_MANAGEMENT_FEES_METRIC = "Performance and Management Fees";
...
async function fetch(options: FetchOptions): Promise<FetchResultV2> {
  ...
  for (let i = 0; i < VaultConfigs[options.chain].length; i++) {
    ...
    if (asset) {
      ...
      const performanceAndManagementFee = yieldTotal - yieldForSupplySide;

      dailyFees.add(asset, yieldForSupplySide, METRIC.ASSETS_YIELDS);
      dailyFees.add(asset, performanceAndManagementFee, PERFORMANCE_AND_MANAGEMENT_FEES_METRIC);
      dailySupplySideRevenue.add(asset, yieldForSupplySide, METRIC.ASSETS_YIELDS);
      dailyRevenue.add(asset, performanceAndManagementFee, PERFORMANCE_AND_MANAGEMENT_FEES_METRIC);
    }
  }
...
const breakdownMethodology = {
  Fees: {
    ...
    [PERFORMANCE_AND_MANAGEMENT_FEES_METRIC]: '20% performance fee charged on the yields generated by the trading strategies.',
  },
  Revenue: {
    [PERFORMANCE_AND_MANAGEMENT_FEES_METRIC]: '20% performance fee charged on the yields, plus 0% management fee on flagship strategy and 2% management fee on other strategies.',
  },
  SupplySideRevenue: {
