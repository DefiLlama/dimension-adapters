...
const getTokenPriceAtTimestamp = async (timestamp: number, tokenAddress: string) => {
  try {
    const pricesRet = await fetchURL(`https://coins.llama.fi/prices/historical/${timestamp}/ethereum:${tokenAddress}`);
    const tokenPriceData = pricesRet.coins[`ethereum:${tokenAddress}`];
    if (!tokenPriceData) {
