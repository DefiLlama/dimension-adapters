...
async function fetchSolana(options: FetchOptions) {
  ...
  const rows = await queryDuneSql(options, `
    ...
    -- creator_trading_fee_percentage is an integer percentage on a 0..100
    -- scale. Meteora settles whole lamports, so the creator share is floored
    -- per swap before aggregation.
    SELECT
      ...
      CAST(COALESCE(SUM(CASE WHEN collect_fee_mode = 1 AND trade_direction = 1 THEN 0 ELSE FLOOR(trading_fee * creator_trading_pct / 100) END), 0) AS VARCHAR) AS creator_fees,
...
const methodology = {
  UserFees:
    "All EVM token-launch fees and Uniswap v4 swap fees paid by users; on Solana, all wSOL-denominated Meteora DBC trading, protocol, and routing-referral fees paid by traders in the three production configs.",
  Fees:
...
const breakdownMethodology = {
  UserFees: {
    [LAUNCH_FEE_LABEL]: "Flat 0.001 ETH fee paid by users on each Base and Robinhood Chain token launch.",
    [SWAP_FEE_LABEL]: "Swap fees paid by traders in ZNS-launched Uniswap v4 and Meteora DBC pools.",
    "Meteora Protocol Fees": "Meteora's protocol share paid by Solana DBC traders.",
    "Meteora Routing Referral Fees": "Referral fees paid from Solana DBC swaps to the submitting frontend or router.",
  },
  Fees: {
