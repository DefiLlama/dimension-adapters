...
const fetchFees = async (options: FetchOptions) => {
    ...
    for (const asset in asset2pools) {
        ...
        const price = pools.map((pool) => pool.tvl).reduce((a, b) => a + b, 0) / pools.map((pool) => pool.reserve).reduce((a, b) => a + b, 0)
        asset_prices[asset] = price
    }
    // explicitly set price for pTON based on TON price
    asset_prices['EQCM3B12QK1e4yZSf8GtBRT0aLMNyEsBc_DhVfRRtOEffLez'] = asset_prices['EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c']
    ...
    console.log(`Requesting last day operations: ${startTime}, ${endTime}`)
...
export default {
    ...
    adapter: {
        [CHAIN.TON]: {
            ...
            meta: {
                methodology: {
                    UserFees: "User pays fee on each swap.",
                    Revenue: "Protocol receives 1/3 of fees paid by users.",
                    SupplySideRevenue: "2/3 of user fees are distributed among LPs.",
                },
            },
            fetch: fetchFees,
        },
