...
const fetch = async (options: FetchOptions) => {
  ...
  const dailyCreatorFees = options.createBalances();
  const data1: any[] = await options.getLogs({
    target: XdockFeeCollector,
    eventAbi: FeeToCreatorEvent,
  });
  data1.forEach((log: any) => {
    dailyCreatorFees.add(WOKB, log.feeAmount);
  });
  return { dailyFees, dailyRevenue: dailyFees, dailyCreatorRevenue: dailyCreatorFees };
};

const methodology = {
  Fees: 'All fees paid by users for launching, trading tokens.',
  Revenue: 'Fees collected by xdock.meme protocol.',
  dailyCreatorRevenue: 'Fees paid to the creator after token graduation',
}
