...
export const fees_bribes = async (fromBlock: number, toBlock: number, timestamp: number): Promise<number> => {
  ...
  const coins = [...new Set(logs_bribes.map((e: IBribes) => `${CHAIN.ZETA}:${e.token.toLowerCase()}`))]
  ...
  const fees_bribes_usd = logs_bribes.map((e: IBribes) => {
    const price = prices[`${CHAIN.ZETA}:${e.token.toLowerCase()}`]?.price || 0;
    const decimals = prices[`${CHAIN.ZETA}:${e.token.toLowerCase()}`]?.decimals || 0;
