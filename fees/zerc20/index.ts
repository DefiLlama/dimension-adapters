...
// Minted amounts (recipient + relayer legs) are therefore gross * 0.9965.
...
const bnb = (balances: any, amount: bigint, label: string) => balances.addCGToken("binancecoin", Number(amount) / 1e18, label);
...
const CONFIG: Record<string, { start: string; assets: Asset[] }> = {
  ...
  [CHAIN.BSC]: { start: "2026-02-02", assets: [{ token: zBNB, liquidityManager: zBNB_LM, add: gas }] },
...
async function fetch(options: FetchOptions) {
  ...
  const { assets } = CONFIG[options.chain];

  if (options.startOfDay >= REDEEM_FEE_LIVE) {
    ...
    teleports.forEach((logs: any[], i: number) => {
      ...
      if (minted > 0n) assets[i].add(dailyFees, (minted * REDEEM_FEE_BPS) / (10000n - REDEEM_FEE_BPS), REDEEM_FEE);
    });
  }
...
const methodology = {
  ...
  UserFees: "Redeem and unwrap fees paid by zERC20 users.",
...
const breakdownMethodology = {
  ...
  UserFees: {
    [REDEEM_FEE]: "0.35% of the gross private-transfer amount, derived from Teleport mints as minted * 0.35 / 99.65.",
    [UNWRAP_FEE]: "feeAmount from LiquidityManager Unwrapped events.",
  },
  Revenue: {
