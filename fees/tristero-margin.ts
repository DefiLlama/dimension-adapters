...
async function readV3LoanValuesAtBlock(
    ...
    positions.forEach((position, index) => {
        const value = toBigIntOrNull(values[index]);
        if (value === null) {
...
function getV3HistoricalValueKey(position: TristeroV3MarginPosition, block: number): string {
...
async function getV3CloseRepayments(
    ...
    receipts.forEach((receipt: any, index) => {
        if (!receipt) {
            throw new Error(`Missing Tristero v3 close receipt for ${options.chain} tx ${normalizeAddress(txHashes[index])}`);
        }

        const txHash = normalizeAddress(receipt.transactionHash ?? receipt.hash);
        if (!txHash) {
            throw new Error(`Missing Tristero v3 close receipt transaction hash for ${options.chain} tx ${normalizeAddress(txHashes[index])}`);
        }
...
const fetch = async (options: FetchOptions): Promise<FetchResultV2> => {
    ...
    if (v3Escrows.length) {
        ...
        const [startLoanValues, endLoanValues, closeRepayments] = await Promise.all([
            readV3LoanValuesByPositionBlock(options, startBlocks),
