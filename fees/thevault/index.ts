...
const fetch = async (_a: any, _b: any, options: FetchOptions) => {
  ...
  results.forEach((row: any) => {
    if (row.metric_type === 'dailyFees') {
      ...
      dailyRevenue.addCGToken("the-vault-staked-sol", row.amount || 0, METRIC.STAKING_REWARDS);
    }
  });
...
const breakdownMethodology = {
  Fees: {
    [METRIC.STAKING_REWARDS]: 'Staking yield rewards generated by the vault',
  },
  Revenue: {
    [METRIC.STAKING_REWARDS]: 'Includes 0.1% fee for delayed unstaking, 5% fee on staking rewards and a 0.1% fee applies when burning LST tokens created through the LST Creator program',
  },
  ProtocolRevenue: {
    [METRIC.STAKING_REWARDS]: 'Includes 0.1% fee for delayed unstaking, 5% fee on staking rewards and a 0.1% fee applies when burning LST tokens created through the LST Creator program',
  }
