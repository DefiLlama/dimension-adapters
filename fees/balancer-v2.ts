...
const v2Endpoints: Record<Chain, string> = {
  [CHAIN.ETHEREUM]: sdk.graph.modifyEndpoint('Fog6Z9z7DXvWy4bx36c7ETQftdtr4Ppxn7Mjpxkzka2i'),
...
const fetch = async ({ createBalances, fromTimestamp, toTimestamp, chain }: FetchOptions) => {
  ...
  const graphUrl = v2Endpoints[chain];
  ...
  try {
    const graphRes: IBalancerSnapshot = await request(graphUrl, graphQuery);
    ...
    dailyFees.addUSDValue(Number(df), METRIC.SWAP_FEES);
    dailySupplySideRevenue.addUSDValue(Number(dailySwapFee), METRIC.LP_FEES);
    dailyRevenue.addUSDValue(Number(dailyProtocolFee), METRIC.PROTOCOL_FEES);
...
const adapter: SimpleAdapter = {
  ...
  fetch,
  adapter: {
    [CHAIN.ETHEREUM]: { start: '2021-04-23' },
    [CHAIN.POLYGON]: { start: '2021-06-24' },
    [CHAIN.ARBITRUM]: { start: '2021-08-31' },
    [CHAIN.AVAX]: { start: '2023-02-25' },
    [CHAIN.XDAI]: { start: '2023-01-10' },
    [CHAIN.BASE]: { start: '2023-07-26' },
    [CHAIN.POLYGON_ZKEVM]: { start: '2023-06-13' },
    [CHAIN.MODE]: { start: '2024-05-22' },
    [CHAIN.FRAXTAL]: { start: '2024-05-20' },
  },
  methodology,
