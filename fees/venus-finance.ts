...
const getContext = async (timestamp: number, _: ChainBlocks): Promise<IContext> => {
  ...
  const allMarketAddressess = await getAllMarkets(unitroller, CHAIN.BSC);
  const { underlyings, reserveFactors } = await getMarketDetails(allMarketAddressess,CHAIN.BSC);

  const prices = await getPrices(
    [
      ...underlyings.filter((e: string) => e).map((x: string) => `${CHAIN.BSC}:${x.toLowerCase()}`),
    ],
    timestamp
  );
...
const getMarketDetails = async (markets: string[], chain: CHAIN): Promise<{underlyings: string[], reserveFactors:string[]}> => {
  ...
  _underlyings[5]  = '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c';
  return {
