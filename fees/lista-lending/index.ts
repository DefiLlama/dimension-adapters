...
// Event ABIs
const vaultFeeClaimedEvent = "event VaultFeeClaimed(address vault, address token, uint256 assets, uint256 shares)"
const marketFeeClaimedEvent = "event MarketFeeClaimed(bytes32 id, address token, uint256 assets, uint256 shares)"

const fetch = async (options: FetchOptions) => {
  const dailyFees = options.createBalances()

  // Get VaultFeeClaimed events
  const vaultFeeLogs = await options.getLogs({ target: eventContract, eventAbi: vaultFeeClaimedEvent, })

  //   Get MarketFeeClaimed events
  const marketFeeLogs = await options.getLogs({ target: eventContract, eventAbi: marketFeeClaimedEvent, })
