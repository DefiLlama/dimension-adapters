...
async function fetch(options: FetchOptions): Promise<FetchResultV2> {
    ...
    dailyFees.add(config.underlying, totalFees.toString(), METRIC.BORROW_INTEREST);
    dailySupplySideRevenue.add(config.underlying, lpYield.toString(), 'Borrow Interest To LP');
    if (protocolFee > 0n) {
        dailyRevenue.add(config.underlying, protocolFee.toString(), 'Borrow Interest To Protocol');
    }
...
const breakdownMethodology = {
    Fees: {
        [METRIC.BORROW_INTEREST]: "Total borrow interest generated by Filecoin InfinityPoolV2 and Base ICN pools",
    },
    Revenue: {
        'Borrow Interest To Protocol': "10% of borrow interest goes to the GLIF protocol treasury (events.glif.link/pool/0/fees).",
    },
    SupplySideRevenue: {
        'Borrow Interest To LP': "90% of SP borrow interest accrues to iFIL and stICNT holders via share-rate appreciation.",
    },
};
