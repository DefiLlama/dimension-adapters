...
async function fetch(options: FetchOptions) {
  ...
  const startSupplies = await options.api.multiCall({
  ...
  for (let i = 0; i < chainVaults.length; i++) {
    ...
    const startSupply = startSupplies[i];
    const endSupply = endSupplies[i];

    if (!startPrice || !endPrice || !params || !startSupply || !endSupply)
      continue;

    const [, asset] = params;

    const paddedVault =
      "0x" + chainVaults[i].slice(2).padStart(64, "0");

    const inflows = await options.api.getLogs({
      target: asset,
      topic: transferTopic,
      topics: [null, null, paddedVault],
      fromBlock,
      toBlock,
    });

    const outflows = await options.api.getLogs({
      target: asset,
      topic: transferTopic,
      topics: [null, paddedVault, null],
      fromBlock,
      toBlock,
    });
    ...
    for (const log of inflows) {
      if (log.data) netInflow += BigInt(log.data);
    }
    ...
    const startValue =
      (BigInt(startPrice) * BigInt(startSupply)) / ONE_18;
    const endValue =
      (BigInt(endPrice) * BigInt(endSupply)) / ONE_18;

    const tradingYield = endValue - startValue - netInflow;
  ...
  return {
    dailyFees,
    dailyRevenue, // intentionally zero
...
const methodology = {
  Fees:
    "Trading PnL generated by Theo straddle vaults. No explicit protocol fee is taken; all PnL accrues to depositors.",
  Revenue:
    "Theo does not publicly disclose protocol fee extraction from straddle vaults.",
  SupplySideRevenue:
    "Vault depositor PnL calculated as (ending vault value − starting vault value − net deposits).",
};

const adapter: SimpleAdapter = {
  version: 2,
  adapter: {
    [CHAIN.ETHEREUM]: { fetch, start: 1701388800 },
    [CHAIN.ARBITRUM]: { fetch, start: 1701388800 },
    [CHAIN.BASE]: { fetch, start: 1710720000 },
    [CHAIN.LINEA]: { fetch, start: 1710720000 },
  },
  methodology,
