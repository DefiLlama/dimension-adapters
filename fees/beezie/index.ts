...
const PAYMENT_TOKENS: Record<string, string[]> = {
  [CHAIN.BASE]: ["0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"], // USDC
  [CHAIN.FLOW]: ["0xd3bF53DAC106A0290B0483EcBC89d40FcC961f3e"], // PYUSD
};
...
const fetchBidRouterVolume = async (options: FetchOptions, bidRouterAddress: string) => {
  const tokens = PAYMENT_TOKENS[options.chain] ?? [];
  ...
  await addTokensReceived({
    ...
    tokens,
    logFilter: (log: any) => !CLAW_MANAGERS.has((log.from ?? log.args?.from ?? "").toLowerCase()),
  });
