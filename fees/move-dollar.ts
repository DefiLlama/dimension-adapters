...
import { getResources } from "../helpers/aptos";
...
const fetch = async (options: FetchOptions) => {
  // last ledger version at or before the end of the window
  const ledgerVersion = await sdk.chains.aptos.getVersionAtTimestamp({ timestamp: options.toTimestamp });
  const resources: AptosResource[] = await getResources(MOVE_DOLLAR_ADDRESS, ledgerVersion);
