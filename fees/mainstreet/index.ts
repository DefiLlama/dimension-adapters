...
const fetch = async (options: FetchOptions) => {
  ...
  dailySupplySideRevenue.addBalances(dailyFees.clone(INSURANCE_SHARE), METRIC.ASSETS_YIELDS);

  const dailyRevenue = dailyFees.clone(TREASURY_SHARE, METRIC.PROTOCOL_FEES);
...
const adapter: Adapter = {
  ...
  breakdownMethodology: {
    ...
    Revenue: {
      [METRIC.PROTOCOL_FEES]: "10% of gross yield allocated to Mainstreet treasury.",
    },
    ProtocolRevenue: {
      [METRIC.PROTOCOL_FEES]: "10% of gross yield allocated to Mainstreet treasury.",
    },
    SupplySideRevenue: {
      [METRIC.ASSETS_YIELDS]: "90% of gross yield distributed to msY holders and Mainstreet's insurance fund reserve.",
    },
  },
};
