...
// BowYard is a USDG-native launchpad and DEX on Robinhood Chain. Classic DEX
// pools are discovered from the Factory's PairCreated event. Agent DEX pools
// are discovered from AgentTokenGraduated. Both discovery paths are cached from
// deployment and automatically include every future graduated market.
//
...
const fetch = async (options: FetchOptions) => {
  ...
  const [created, curveTrades, agentCurveTrades, classicPairs, agentPairs] = await Promise.all([
    options.getLogs({ target: LAUNCHPAD, eventAbi: TOKEN_CREATED }),
  ...
  for (const trade of agentCurveTrades) {
    addAgentFee(
