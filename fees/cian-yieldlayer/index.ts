...
const fetch = async ({createBalances, api, fromApi, toApi, chain}: FetchOptions) => {
    ...
    const vaultsTotalAssets = await fromApi.multiCall({
        abi: cianVaultABI.totalAssets,
        calls: vaults
    })

    const vaultsTotalAssetsEnd = await toApi.multiCall({
        abi: cianVaultABI.totalAssets,
        calls: vaults
    })

    vaultsTotalAssetsEnd.forEach((totalAssets, i) => {
        dailyFees.add(vaultsParams[i].token, (totalAssets - vaultsTotalAssets[i]) * vaultsParams[i].managementFeeRate / 1e4)
        dailyFees.add(vaultsParams[i].token, (totalAssets - vaultsTotalAssets[i]) * vaultsParams[i].revenueRate / 1e4)
    })
