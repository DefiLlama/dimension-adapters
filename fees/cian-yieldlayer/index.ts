...
const cianVaults: Record<string, string[]> = {
    [CHAIN.ETHEREUM]: [
...
const fetch = async ({createBalances, api, getLogs, fromApi, toApi, chain}: FetchOptions) => {
    ...
    const vaultsUpdateExchangePrice = (await getLogs({
        ...
        return logs.reduce((totalRevenue: bigint, log: any) => totalRevenue + log.newRevenue, 0n);
    });
    ...
    const vaultsManagementFee = (await getLogs({
        ...
        return logs.reduce((totalManagement: bigint, log: any) => totalManagement + log.managementFee, 0n);
    });
    ...
    vaultsUpdateExchangePrice.forEach((data, i) => {
        ...
        dailyFees.add(validVaults[i].token, yieldValue, METRIC.ASSETS_YIELDS);
        dailySupplySideRevenue.add(validVaults[i].token, yieldValue - data, "Asset Yields to Vault Depositors");
        dailyRevenue.add(validVaults[i].token, data, METRIC.PERFORMANCE_FEES);
    });
    
    vaultsManagementFee.forEach((data, i) => {
        if(validVaults[i].managementFeeRate == 0) return;
        dailyFees.add(validVaults[i].token, data, METRIC.MANAGEMENT_FEES);
        dailyRevenue.add(validVaults[i].token, data, METRIC.MANAGEMENT_FEES);
    });
...
const breakdownMethodology = {
  Fees: {
    [METRIC.ASSETS_YIELDS]: "Yield generated by the vaults",
  ...
  SupplySideRevenue: {
    "Asset Yields to Vault Depositors": "Yield returned to vault depositors",
  },
}
