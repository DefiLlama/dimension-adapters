...
const chainConfig: Record<string, Record<string, string>> = {
    [CHAIN.ETHEREUM]: { formattedName: "Ethereum", start: "2025-01-01" },
    [CHAIN.BSC]: { formattedName: "Binance", start: "2025-01-01" },
    [CHAIN.AVAX]: { formattedName: "Avalanche", start: "2025-01-01" }
}

const multipliTokenMap: Record<string, any> = {
    xusdc: {
        underlying: {
            [CHAIN.ETHEREUM]: "USDC", [CHAIN.BSC]: "USDC", [CHAIN.AVAX]: "AVALANCHEUSDC"
        },
        coingeckoId: "usd-coin"
    },
    xusdt: {
        underlying: {
            [CHAIN.ETHEREUM]: "USDT", [CHAIN.BSC]: "USDT"
        },
        coingeckoId: "tether"
    },
    xwbtc: {
        underlying: {
            [CHAIN.ETHEREUM]: "WBTC", [CHAIN.BSC]: "WBTC", [CHAIN.AVAX]: "BTC.B"
        },
...
async function fetch(_a: any, _b: any, options: FetchOptions): Promise<FetchResult> {
    ...
    const tokensTvlToday = tvlData.chainTvls[formattedChainName].tokens.find((entry: any) => entry.date === options.startOfDay).tokens;
    ...
    for (const [xToken, tokenDetails] of Object.entries(multipliTokenMap)) {
        ...
        const latestApy = yieldsData.payload[xToken][0].data[totalYieldEntries - 1].value;

        const yieldForPeriod = (latestTvl * latestApy * periodWrtYear) / 100;
        dailyFees.addCGToken(tokenDetails.coingeckoId, yieldForPeriod,METRIC.ASSETS_YIELDS);
    }
...
const methodology = {
    ...
    Revenue: "No deposit or withdrawal fees, no transparent mention of performance/management fees",
