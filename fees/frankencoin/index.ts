...
const fetch = async (options: FetchOptions) => {
  ...
  `;
	...
	const accumRevenue =
		accumLosses >= accumProfits ? 0n : accumProfits - accumLosses;

	// Fees are the total accumProfits. What users pay upfront.
	dailyFees.add(ZCHF_ADDRESS, accumProfits);

	// Revenue are in ZCHF
	dailyRevenue.add(ZCHF_ADDRESS, accumRevenue);
	dailyProtocolRevenue.add(ZCHF_ADDRESS, accumRevenue);

	// Interest/staking paid by the protocol
