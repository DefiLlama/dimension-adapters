...
async function getFeeRevenueData(
  ...
  const dailyFees = (toNumber(response.today?.feeRevenueUSD || '0') - toNumber(response.yesterday?.feeRevenueUSD || '0'))/1e12;
  const dailyProtocolFees = (toNumber(response.today?.protocolFeeRevenueUSD || '0')  - toNumber(response.yesterday?.protocolFeeRevenueUSD || '0'))/1e12;
  ...
  const totalFees = (toNumber(response.factories[0]?.feeRevenueUSD || '0'))/1e12;
  const totalProtocolFees = (toNumber(response.factories[0]?.protocolFeeRevenueUSD || '0'))/1e12;
