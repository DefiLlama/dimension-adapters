...
const fetch = async (options: FetchOptions) => {
    ...
    WITH initializations AS (
      ...
      SELECT p.quote_mint,
             ...
             COUNT_IF(t.platform_fee IS NULL OR t.protocol_fee IS NULL OR t.creator_fee IS NULL
               OR p.quote_count <> 1 OR p.quote_mint IS NULL) AS invalid_rows
  ...
  const raw = (value: unknown): string => {
    if (typeof value !== 'string' || !/^\d+$/.test(value)) throw new Error('Invalid Pairz raw fee amount');
    return value;
  };
  for (const row of rows) {
    ...
    if (row.quote_mint === null && row.invalid_rows === null) continue;
