...
const calculateGweiFees = (side: string, volume: bigint, amountCoinA: bigint) => {
    ...
    if (side === 'buy') {
        const ethVolume = (calculateSaleReturn(volume, totalCoins) * 10000n) / 9900n;
        return ethVolume * 100n / 10000n;
    } else {
        const ethVolume = (calculateSaleReturn(volume, totalCoins + volume) * 9900n) / 10000n;
        return ethVolume * 100n / 9900n;
    }
};
