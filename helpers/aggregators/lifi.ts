...
export const fetchVolumeFromLIFIAPI = async (chain: Chain, start: number, endTime: number, integrators?: string[], exclude_integrators?: string[], swapType?: 'cross-chain' | 'same-chain'): Promise<number> => {
  ...
  while (hasMore) {
    ...
    transfers.forEach((tx) => {
      if (
        ...
        // runs we must drop any transfer whose timestamp falls outside [start, endTime]
        tx.sending.timestamp >= start && tx.sending.timestamp < endTime &&
