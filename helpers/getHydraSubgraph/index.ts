...
function getGraphDimensions({
  ...
  // Queries
  // check if it's v2 graph and change the block number type
  const totalVolumeQuery = gql`
  query total_volume ($block: ${graphFieldsTotalVolume.factory.includes('hydraswap') ? 'Float!' : 'Int'}) {
