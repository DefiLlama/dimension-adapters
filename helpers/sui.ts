...
export async function queryEventsByModule({ package: pkg, module: mod, options, transform = (i:any) => i }:any):Promise<any[]> {
  const filter = { MoveEventModule: { package: pkg, module: mod } }
  const items:any[] = []
  let cursor = null
  do {
