...
const fetch = async (options: FetchOptions) => {
  ...
  dailyHoldersRevenue.addUSDValue(totalFees * (0.05 / 0.3), 'Energy Holder Fees');
...
const methodology = {
  ...
  HoldersRevenue: "0.05% of each swap is distributed to all accounts holding Energy (xMEX lockers).",
...
const adapter: SimpleAdapter = {
  ...
  breakdownMethodology: {
    ...
    HoldersRevenue: {
      'Energy Holder Fees': "0.05% of swap fees distributed to all accounts holding Energy (xMEX lockers).",
    },
    SupplySideRevenue: {
      [METRIC.LP_FEES]: "0.2% of swap fees distributed to liquidity providers proportional to their pool share.",
    },
  },
};
