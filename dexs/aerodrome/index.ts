...
const getBribes = async (fetchOptions: FetchOptions): Promise<{ dailyBribesRevenue: sdk.Balances }> => {
  ...
  const bribes_contract: string[] = logs_gauge_created.map((log) => log.bribeVotingReward.toLowerCase())
...
const getVolumeAndFees = async (fromBlock: number, toBlock: number, fetchOptions: FetchOptions) => {
  ...
  const rawPools = await fetchOptions.getLogs({ target: CONFIG.factory, fromBlock: 3200668, eventAbi: eventAbis.event_pool_created, onlyArgs: true, cacheInCloud: true, skipIndexer: true, })

  const fees = await api.multiCall({
    abi: abis.fees, target: CONFIG.factory, calls: rawPools.map(i => ({ params: [i.pool, i.stable] }))
  })
