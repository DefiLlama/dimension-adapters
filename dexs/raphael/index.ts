...
const config: Record<string, ChainCfg> = {
  [CHAIN.ROBINHOOD]: {
    factory: RAPH_FACTORY, start: '2026-08-31',
    gaugeCreated: { voter: RAPH_VOTER, fromBlock: 51041709, abi: gaugeCreatedEvent, bribeField: 'bribeVotingReward' },
  },
}
...
const collectGaugesAndBribes = async (options: FetchOptions, cfg: ChainCfg) => {
  ...
  // Voter can be shared with a CL factory — keep only this factory's gauges so
  // CL bribes don't leak into v2 fees.
...
const fetch = async (options: FetchOptions) => {
  ...
  // Prefer the TVL adapter's cached pair list (same as uniV2Exports). Raphael has
  // no TVL adapter yet, so fall back to enumerating pools straight from the
  // PoolFactory (Velodrome-v2 ABI: allPoolsLength / allPools — allPairs* revert).
  ...
  if (!pairs?.length) throw new Error('No pairs found for Raphael')
...
const adapter: SimpleAdapter = {
  ...
  adapter: config,
  methodology: {
    Volume: 'Swap volume across Raphael Exchange (non-CL) pools, counted once per swap from the core-asset side.',
    ...
    Revenue: 'veRAPH voters’ take: staked-LP swap fees forwarded to voters plus external bribes. Equals HoldersRevenue — Raphael keeps no treasury cut of swap fees.',
    HoldersRevenue: 'Swap fees from staked LPs (fee × pool.balanceOf(gauge) / pool.totalSupply), forwarded to veRAPH voters, plus external bribes.',
    SupplySideRevenue: 'Swap fees kept by unstaked LPs (fee × (1 − stakedShare)). LPs who stake in the gauge forgo these fees in exchange for RAPH emissions.',
  },
  breakdownMethodology: {
    ...
    Revenue: {
      'Staked-LP Swap Fees': 'Staked-LP share of swap fees, forwarded to veRAPH voters.',
