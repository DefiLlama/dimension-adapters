...
const PERP_ORDERBOOK_REGISTERED = "event PerpOrderBookRegistered(address orderBook, uint32 buyTokenId, uint32 payTokenId)";
...
async function getPerpOrderbooks(
  ...
  const perpLogs = await getLogs({
    ...
    eventAbi: PERP_ORDERBOOK_REGISTERED,
    fromBlock: fromBlock,
    cacheInCloud: false,
    skipIndexer: true,
  });
  ...
  perpLogs.forEach((log: any) => {
    ...
    if (orderbookAddr && buyTokenId && payTokenId) {
      ...
      orderbookConfigs[orderbookAddr] = { baseId: buyTokenId, quoteId: payTokenId, type: "PERP" };
