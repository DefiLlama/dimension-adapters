    ...
    const untrackVolumes: number[] = lpTokens.map(
      (_: string, index: number) => {
        const token0Decimals =
          prices[`${CHAIN.CANTO}:${tokens0[index]}`]?.decimals || 0
        const token1Decimals =
          prices[`${CHAIN.CANTO}:${tokens1[index]}`]?.decimals || 0
        ...
        const token0Price =
          prices[`${CHAIN.CANTO}:${tokens0[index]}`]?.price || 0
        const token1Price =
          prices[`${CHAIN.CANTO}:${tokens1[index]}`]?.price || 0
