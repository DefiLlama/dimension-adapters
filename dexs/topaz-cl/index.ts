...
import { handleBribeToken } from "../topaz/utils";
...
const getBribes = async (fetchOptions: FetchOptions): Promise<{ dailyBribesRevenue: sdk.Balances }> => {
  ...
  const logs_gauge_created = await getLogs({ target: CONFIG.voter, fromBlock: 98741567, eventAbi: eventAbis.event_gaugeCreated, cacheInCloud: true, })
...
const fetch = async (_: any, _1: any, fetchOptions: FetchOptions): Promise<FetchResult> => {
  ...
  const rawPools = await getLogs({ target: CONFIG.factory, fromBlock: 98741567, toBlock, eventAbi: eventAbis.event_poolCreated, cacheInCloud: true, })
  ...
  await PromisePool
    ...
    .process(async ([startBlock, endBlock]: any) => {
      ...
      try {
        ...
        sdk.log(`Topaz cl got logs (${logs.length}) for ${i++}/ ${Math.ceil((toBlock - fromBlock) / blockStep)}`)
        logs.forEach((log: any) => {
