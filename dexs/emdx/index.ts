...
const getVolumes = async (): Promise<IData[]> => {
  ...
  return response.data.data.get_execution.execution_succeeded?.data
}

const fetch = async (timestamp: number) => {
  const dayTimestamp = getUniqStartOfTodayTimestamp(new Date((timestamp * 1000)))
  const info = await getVolumes();
  const volume: (IData | undefined) = info.find((e: IData) =>  (new Date(e.day.split('T')[0]).getTime() / 1000) === dayTimestamp);
  return {
