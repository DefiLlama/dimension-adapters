...
function fromDuneChain(duneChainId: string): string {
  if (duneChainId === 'bnb') return CHAIN.BSC;
  if (duneChainId === 'avalanche_c') return CHAIN.AVAX;
  if (duneChainId === 'worldchain') return CHAIN.WC;
...
async function fetch(options: FetchOptions) {
  ...
  if (Configs[options.chain].source === "LOGS") {
    ...
    const tokens = options.preFetchedResults ? options.preFetchedResults
      .map((item: any) => {
        return {
          ...item,
          chain: fromDuneChain(item.chain),
        }
      })
      .filter((item: any) => item.chain === options.chain)
    : [];

    for (const token of tokens) {
      if (token === '0x0000000000000000000000000000000000000000') {
        dailyFees.addGasToken(token.totalSwapFee)
        dailyVolume.addGasToken(token.totalSwapAmount)
      } else {
        dailyFees.add(token.token, token.totalSwapFee)
        dailyVolume.add(token.token, token.totalSwapAmount)
      }
    }
  }
