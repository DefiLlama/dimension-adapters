...
async function fetch(options: FetchOptions) {
  ...
  if (config.source === 'LOGS') {
    events = await options.getLogs({
      target: config.poolManager,
      eventAbi: SwapEvent,
    });
  } else if (config.source === 'INDEXER') {
    events = (await sdk.indexer.getLogs({
      chain: options.chain,
      onlyArgs: true,
      fromBlock: options.fromApi.block as number,
      toBlock: options.toApi.block as number,
      eventAbi: SwapEvent,
      target: config.poolManager,
    })).map(item => item);
  }
  ...
  for (let i = 0; i < poolIds.length; i++) {
    if (poolKeys[i]) {
      if (String(poolKeys[i].currency0) !== NativeCoin || String(poolKeys[i].currency1) !== NativeCoin) {
        pools[poolIds[i]] = {
          poolId: poolIds[i],
