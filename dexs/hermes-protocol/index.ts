...
const fetch = async (timestamp: number) => {
  ...
  const untrackVolumes: number[] = lpTokens.map((_: string, index: number) => {
    ...
    const token0Price = (prices[`metis:${tokens0[index].toLowerCase()}`]?.price || 0);
    const token1Price = (prices[`metis:${tokens1[index].toLowerCase()}`]?.price || 0);
    const token0Decimals = (prices[`metis:${tokens0[index].toLowerCase()}`]?.decimals || 0)
    const token1Decimals = (prices[`metis:${tokens1[index].toLowerCase()}`]?.decimals || 0)
