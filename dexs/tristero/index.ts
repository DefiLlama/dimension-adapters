...
function addTristeroV3CloseReceiptsToBalances(
  ...
  receipts.forEach((receipt, index) => {
    ...
    const loanAssets = [...new Set(closePositions.map((position) => normalize(position.loanAsset)))];
    loanAssets.forEach((loanAsset) => {
      ...
      (receipt.logs ?? []).forEach((log) => {
        ...
        if (
          ...
          || topicToAddress(topics[2]) === "0x0000000000000000000000000000000000000000"
          || !isNonZeroBytes32(log.data)
        ) {
          return;
        }
