...
const fetch = async (options: FetchOptions) => {
  ...
  allLogs.forEach((logs: any, index: number) => {
    ...
    logs.forEach((log: any) => {
      ...
      const { token, amount } = addOneToken({ chain, balances: dailyFees, token0, token1, amount0: log.amount0.toString() * feeTier, amount1: log.amount1.toString() * feeTier });
      ...
      dailyRevenue.add(token, amount * protocolShare);
      dailySupplySideRevenue.add(token, amount * (1 - protocolShare));
    });
  });
