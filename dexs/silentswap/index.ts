...
async function readInflowAtDate(api: FetchOptions["api"], date: bigint): Promise<bigint> {
    ...
    // The decoded named tuple has `inflowUsd` at index 1; fall back to positional.
    const inflow =
...
const adapter: SimpleAdapter = {
    ...
    adapter: {
        [CHAIN.BSC]: {
            fetch,
            start: "2026-05-25",
            // The reporter stores per-date snapshots as monotonic current state
            // (`volumeByDate[date]` is written once and never overwritten), so
            // reading at the latest block returns correct historical day values
            // without needing an archive node.
            runAtCurrTime: true,
            meta: {
                methodology: {
                    Volume:
                        "Anonymized inflow series read from the SilentSwapStats " +
