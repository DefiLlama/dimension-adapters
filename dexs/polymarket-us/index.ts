...
async function fetch(_a: any, _b: any, options: FetchOptions) {
    ...
    const isTodaysDataAvailable = manifestData.files.find((item: any) => item.filename === `${options.dateString.replaceAll('-', '')}-time-and-sales.csv`);
...
const methodology = {
    dailyVolume: 'The total volume of trades on Polymarket US',
    dailyNotionalVolume: 'The total notional volume of trades on Polymarket US',
    dailyFees: 'Users pay fees when they trade binary options on Polymarket US',
    dailyUserFees: 'Users pay fees when they trade binary options on Polymarket US',
    dailyRevenue: 'The total revenue generated by Polymarket US after rebates are distributed',
    dailyProtocolRevenue: 'All the revenue goes to protocol',
    dailySupplySideRevenue: 'Part of fees distributed to traders',
}
