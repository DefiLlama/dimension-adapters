...
async function fetchV2(fetchOptions: FetchOptions) {
  ...
  const token1s = await api.multiCall({ abi: 'function connectorTokens(uint256) view returns (address)', calls: pools.map(i => ({ target: i, params: [1] })) })
  const token0s = await api.multiCall({ abi: 'function connectorTokens(uint256) view returns (address)', calls: pools.map(i => ({ target: i, params: [0] })) })
