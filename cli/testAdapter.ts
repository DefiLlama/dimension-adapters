...
import runAdapter, {
  getFulfilledResults,
  getRejectedResults,
} from "../adapters/utils/runAdapter";
...
const passedFile = path.resolve(
  process.cwd(),
  `./${adapterType}/${process.argv[3]}`
);
(async () => {
  try {
    const cleanDayTimestamp = process.argv[4]
      ? getUniqStartOfTodayTimestamp(
          new Date(+process.argv[4] * 1000 + 60 * 60 * 24 * 1000)
        )
    ...
    console.info(
      `${upperCaseFirst(adapterType)} for ${formatTimestampAsDate(
        String(
          getUniqStartOfTodayTimestamp(new Date(endCleanDayTimestamp * 1000))
        )
      )}`
    );
    console.info(`_______________________________________\n`);
      ...
      allChains.map(async (chain) => {
        try {
          const latestBlock = await getBlock(
            endCleanDayTimestamp,
            chain,
            chainBlocks
          ).catch((e: any) =>
            console.error(`${e.message}; ${endCleanDayTimestamp}, ${chain}`)
          );
    ...
    if ("adapter" in module) {
      ...
      const allVolumes = await Promise.all(
        Object.entries(breakdownAdapter).map(
          async ([version, adapter]) =>
            await runAdapter(adapter, cleanDayTimestamp, chainBlocks).then(
              (res) => ({ version, res })
            )
        )
      );
